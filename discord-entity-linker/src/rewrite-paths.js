...
export function getEntityOutputBaseUrl() {
  ...
  if (!override) return ENTITY_OUTPUT_BASE_URL;
...
function splitTrailingPunctuation(relativePath) {
  ...
  if (!trailingPunctuation) return { path: normalizedPath, trailingPunctuation: "" };
  return { path: normalizedPath.slice(0, -trailingPunctuation.length), trailingPunctuation };
}
...
export function replaceEntityPaths(text) {
  ...
  for (const { regex } of PATTERNS) {
    ...
    result = result.replace(re, (_match, relativePath) => {
      ...
      if (!trailingPunctuation) return url;
